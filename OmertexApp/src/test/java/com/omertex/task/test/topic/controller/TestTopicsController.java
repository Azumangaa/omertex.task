...
public class TestTopicsController 
{
	...
	public void shouldShowTopics() throws Exception
	{
		...
		MockMvc mvc = standaloneSetup(controller)
				.setSingleView(new InternalResourceView("/WEB-INF/jsp/topics.jsp"))
				.build();
		...
		mvc.perform(get("/topics/1"))
			...
			.andExpect(model().attributeExists("beginPage"))
			.andExpect(model().attribute("beginPage", Math.max(1, page.getNumber() + 1 - 5)))
			.andExpect(model().attributeExists("endPage"))
			.andExpect(model().attribute("endPage", Math.min(Math.max(1, 
					page.getNumber() + 1 - 5) + 10, page.getTotalPages())));
	}
